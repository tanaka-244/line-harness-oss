 ...
 * 既に和暦の場合はそのまま返す
 */
export function convertToWareki(dateStr: string): string {
  if (!dateStr) return '';

  const wareki = ['明治', '大正', '昭和', '平成', '令和'];
  for (const era of wareki) {
    if (dateStr.includes(era)) return dateStr;
  }
  ...
  if (year >= 2019) {
    if (year === 2019 && month < 5) {
      eraYear = 31; eraName = '平成';
    } else {
      eraYear = year - 2018; eraName = '令和';
    }
  } else if (year >= 1989) {
    if (year === 1989 && month === 1 && day < 8) {
      eraYear = 64; eraName = '昭和';
    } else {
      eraYear = year - 1988; eraName = '平成';
    }
  } else if (year >= 1926) {
    if (year === 1926 && (month < 12 || (month === 12 && day < 25))) {
      eraYear = 15; eraName = '大正';
    } else {
      eraYear = year - 1925; eraName = '昭和';
    }
  } else if (year >= 1912) {
    if (year === 1912 && (month < 7 || (month === 7 && day < 30))) {
      eraYear = 45; eraName = '明治';
    } else {
      eraYear = year - 1911; eraName = '大正';
    }
  } else {
    eraYear = year - 1867; eraName = '明治';
  }
 ...
 */
export function convertRelativeDate(dateStr: string, baseDate?: Date): string {
  if (!dateStr) return '';
  ...
  if (/\d{1,2}月\d{1,2}日/.test(dateStr)) return dateStr;
