...
// mm → points 変換
export const pt = (mm) => mm * 2.8346;
// 共通色
export const BLACK = rgb(0, 0, 0);
// ---------------------- 共通描画ヘルパー ----------------------
/** 外枠付き矩形（fill なし） */
export function drawRect(page, x, y, w, h, lineWidth = 0.8) {
    page.drawRectangle({ x, y, width: w, height: h, borderColor: BLACK, borderWidth: lineWidth });
}
/** 水平線 */
export function drawHLine(page, x1, x2, y, thickness = 0.3) {
    page.drawLine({ start: { x: x1, y }, end: { x: x2, y }, thickness, color: BLACK });
}
/** 垂直線 */
export function drawVLine(page, x, y1, y2, thickness = 0.3) {
    page.drawLine({ start: { x, y: y1 }, end: { x, y: y2 }, thickness, color: BLACK });
}
/** テキスト描画（baseline基準） */
export function drawText(page, text, x, y, font, size) {
    if (!text)
        return;
    page.drawText(text, { x, y, font, size, color: BLACK });
}
/** 右寄せテキスト */
export function drawTextRight(page, text, rightX, y, font, size) {
    if (!text)
        return;
...
/** 中央寄せテキスト */
export function drawTextCenter(page, text, centerX, y, font, size) {
    if (!text)
        return;
    const w = font.widthOfTextAtSize(text, size);
 ...
 * 日本語1文字ずつチェックして折り返し
 */
export function splitLines(text, font, size, maxWidth) {
    if (!text)
        return [];
 ...
 */
export function drawNameRow(page, font, marginLeft, currentY, contentWidth, data, opts) {
    const rowH = pt(18);
 ...
 * @returns 次のy座標
 */
export function drawNameRowBeauty(page, font, marginLeft, currentY, contentWidth, data) {
    const rowH = pt(18);
 ...
 */
export function drawAddressRow(page, font, marginLeft, currentY, contentWidth, data) {
    const rowH = pt(15);
 ...
 * @returns 次のy座標
 */
export function drawConsentSection(page, font, marginLeft, currentY, contentWidth, notices) {
 ...
 * @returns 次のy座標
 */
export function drawOtherClinic(page, font, marginLeft, currentY, data, mode) {
 ...
 * @returns 次のy座標
 */
export function drawMedicalHistory(page, font, marginLeft, currentY, contentWidth, data, opts) {
    drawText(page, '他に今までにあった病気やアレルギー、ペースメーカー、妊娠中、気になることなどあればご記入ください', marginLeft, currentY, font, 10);
    currentY -= pt(3);
    const boxH = pt(18);
 ...
 * @returns 次のy座標
 */
export function drawReferral(page, font, marginLeft, currentY, data) {
 ...
 * 施術者記入欄（ページ下まで）
 */
export function drawMemoSection(page, font, marginLeft, currentY, contentWidth) {
 ...
 * @returns 次のy座標
 */
export function drawHeader(page, font, marginLeft, currentY, title, logoImage) {
 ...
 * 人体図を右側に描画
 */
export function drawBodyFigure(page, figureImage, marginLeft, sectionStartY, contentWidth, heightMm = 92) {
    if (!figureImage)
        return;
