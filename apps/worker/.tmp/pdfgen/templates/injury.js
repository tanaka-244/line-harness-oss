...
import { PAGE_W, PAGE_H, pt, drawText, drawRect, drawHLine, drawVLine, drawConsentSection, drawOtherClinic, drawMedicalHistory, drawReferral, drawMemoSection, drawBodyFigure, splitLines, } from './base.js';
import { convertToWareki, convertRelativeDate } from '../utils/wareki.js';
import { getZipcodeFromAddress } from '../utils/zipcode.js';
export async function generateInjuryPdf(answers, fontBytes, logoBytes, figureBytes) {
    ...
    // === ヘッダー ===
    if (logoImage) {
        page.drawImage(logoImage, { x: marginLeft, y: currentY - pt(18), width: pt(18), height: pt(18) });
    }
    drawText(page, '問診票', PAGE_W / 2 - pt(30), currentY - pt(8), font, 24);
    const dateX = PAGE_W / 2 + pt(20);
    drawText(page, '来院日', dateX, currentY - pt(5), font, 11);
    drawText(page, '年', dateX + pt(25), currentY - pt(5), font, 11);
    drawText(page, '月', dateX + pt(40), currentY - pt(5), font, 11);
    drawText(page, '日', dateX + pt(55), currentY - pt(5), font, 11);
    currentY -= pt(18);
